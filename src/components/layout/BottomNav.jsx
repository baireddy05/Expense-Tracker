      ...
      <div className="flex justify-around items-center h-14 px-1.5">
        {navItems.map((item) => (
          <NavLink
            ...
            to={item.path}
            className={({ isActive }) =>
              `flex flex-col items-center justify-center flex-1 py-1 rounded-2xl transition-all duration-200 touch-feedback relative ${
                isActive 
          ...
          >
            {({ isActive }) => (
              <>
                <FontAwesomeIcon icon={item.icon} className="text-sm mb-0.5" />
                <span className="text-[10px] tracking-tight">{item.name}</span>
                {isActive && (
                  <span className="w-1.5 h-1.5 rounded-full bg-zinc-900 dark:bg-white mt-0.5 shadow-xs" />
                )}
              </>
            )}
          </NavLink>
