...
const BorrowRepaymentModal = ({ isOpen, onClose, record }) => {
  const { recordBorrowedRepayment } = useTransactions();
  ...
  const handleSubmit = async (e) => {
    ...
    if (parsedAmount > remainingAmount) {
      toast.error(`Repayment cannot exceed remaining debt balance (₹${remainingAmount.toLocaleString('en-IN')})`);
      return;
    }

    setLoading(true);
    try {
      await recordBorrowedRepayment(record.id, {
        amount: parsedAmount,
        date,
        note: note.trim() || 'Repayment'
      });
      toast.success(`Recorded ₹${parsedAmount.toLocaleString('en-IN')} repaid to ${record.lenderName}!`);
      onClose();
    } catch (err) {
      console.error(err);
        ...
        <form onSubmit={handleSubmit} className="flex-1 min-h-0 overflow-y-auto sm:overflow-visible p-5 space-y-4 pb-8 sm:pb-5">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3.5 py-2 liquid-glass-input rounded-xl focus:ring-1 focus:ring-zinc-400 dark:focus:ring-zinc-600 outline-none text-xs dark:text-white bg-zinc-50 dark:bg-zinc-800/80"
            />
          </div>
...
export default BorrowRepaymentModal;
